/*
 Source File : ConstantLight
 
 
 Copyright 2013 Gal Kahana ConstantLight
...
Handle<Value> CalculateBrightness(const Arguments& args)
{
    HandleScope scope;
	if (args.Length() != 1 || !args[0]->IsString())
    {
		ThrowException(Exception::TypeError(String::New("Wrong arguments, provide a path to a jpg image file")));
		return scope.Close(Undefined());
	}
    
    Handle<Value> result = Number::New(calculateImageBrightnessFactor(*String::Utf8Value(args[0]->ToString())));

    return scope.Close(result);
}

void ConstantLightInit(Handle<Object> exports) {
    exports->Set(String::NewSymbol("calculateBrightness"),FunctionTemplate::New(CalculateBrightness)->GetFunction());
}

NODE_MODULE(constantlight, ConstantLightInit)