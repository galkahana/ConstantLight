...
#define DEFAULT_BUFFER_SIZE 256*1024
	...
	public:
		...
		*/
		InputBufferedStream(LongBufferSizeType inBufferSize);

		/*
			Constructor with assigning. see Assign for unassign instructions
		*/
		InputBufferedStream(IByteReaderWithPosition* inSourceReader,LongBufferSizeType inBufferSize = DEFAULT_BUFFER_SIZE);
	...
	private:
		...
		Byte* mLastAvailableIndex;
		IByteReaderWithPosition* mSourceStream;

		void Initiate(IByteReaderWithPosition* inSourceReader,LongBufferSizeType inBufferSize);
